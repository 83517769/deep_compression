...
misc.logger.init(args.logdir, 'log_2021_3_9_mobilenet_v2')
print = misc.logger.info
os.makedirs('mobilenet_v2_models3-9', exist_ok=True)
...
test_l = DataLoader(test_dataset, batch_size=64, shuffle=False, drop_last=True)
...
if args.epoch_ != 0:
    ...
    print(">>>>loading model........<<<<")
    model.load_state_dict(torch.load('mobilenet_v2_models3-9/model_90.pth'))
else:
    model = mobilenet_v2.mobilenet_v2(pretrained=True)
    # model = squeezenet._squeezenet("1_0")
...
for epoch in range(args.n_epochs):
    ...
    batch = tqdm(test_l, total=len(test_dataset) // 64)
    ll = []
    for x, target in batch:
        x = x.to(device)
        target = target.to(device)
    ...
    if epoch % 10 == 0:
        torch.save(model.state_dict(), 'mobilenet_v2_models3-9/model_%d.pth' % epoch)